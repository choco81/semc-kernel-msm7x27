...
static struct flash_identification supported_flash[] =
{
	...
	{0x5500bcec, 0xFF00FFFF,  (512<<20), 1, 2048, (2048<<6),  64},
	{0x5501B3EC, 0xFFFFFFFF, (1024<<20), 1, 2048, (2048<<6),  64},
	/* Toshiba */
	{0x1500aa98, 0xFFFFFFFF,  (256<<20), 0, 2048, (2048<<6),  64},
	{0x5500ba98, 0xFFFFFFFF,  (256<<20), 1, 2048, (2048<<6),  64},
	{0x5591b398, 0x0000FFFF, (1024<<20), 1, 2048, (2048<<6),  64},
	/* Micron */
	...
	/* Numonyx */
	{0x5510bc20, 0xFFFFFFFF,  (512<<20), 1, 2048, (2048<<6),  64},
	{0x5551b320, 0xFFFFFFFF, (1024<<20), 1, 2048, (2048<<6),  64},
...
static int __devinit msm_nand_probe(struct platform_device *pdev)
{
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "msm_nand_phys");
	if (!res || !res->start) {
...
out_free_dma_buffer:
	dma_free_coherent(NULL, MSM_NAND_DMA_BUFFER_SIZE, info->msm_nand.dma_buffer,
